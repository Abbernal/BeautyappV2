...
/**
 * Activity para editar el perfil del cliente.
 * 
 * Permite a los clientes modificar sus datos personales: nombre, email,
 * teléfono y contraseña. Valida que el email no esté ya en uso por otro
 * usuario y que los datos cumplan con los requisitos establecidos.
 * 
 * Contexto de uso: Se accede desde ClienteHomeActivity cuando el cliente
 * hace click en el botón de perfil del toolbar.
 * 
 * Funcionalidades:
 * - Editar nombre completo
 * - Editar email (con validación de formato y unicidad)
 * - Editar teléfono (solo números)
 * - Cambiar contraseña (opcional, mínimo 6 caracteres)
 * - Validaciones:
 *   - Nombre: No puede estar vacío
 *   - Email: Formato válido y no duplicado
 *   - Teléfono: Solo números, no vacío
 *   - Contraseña: Opcional, pero si se proporciona debe tener mínimo 6 caracteres
 * - Actualiza la sesión con los nuevos datos
 * 
 * Flujo: EditarPerfilClienteActivity → (guardar) → ClienteHomeActivity
 * 
 * @author Antonio Benitez
 * @version 1.0
 */
public class EditarPerfilClienteActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (appBar != null) {
            setupAppBar(getString(R.string.client_profile_title));
        }
